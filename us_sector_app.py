...
import time
import pytz
from us_sector_data_v2 import fetch_us_sector_metrics

# ----- ページ設定 -----
# app.py で一括設定するためここでは削除またはコメントアウト
# st.set_page_config(page_title="🇺🇸 全11業種 詳細データテーブル", page_icon="🇺🇸", layout="wide")
...
def main():
    ...
    def load_data():
        return fetch_us_sector_metrics()
    
    # 現在時刻のステータス取得（NY時間の代わりに米国時間）
    def get_market_status():
        jst = pytz.timezone('US/Eastern')
        ...
        if now.time() < open_time:
            ...
            return "🟢 米国市場 取引時間中 (約5分ごとに自動更新)"
        else:
            return "🔴 米国市場 クローズ (本日の確定データ)"
    ...
    <div style="background-color: #1E1E1E; padding: 20px; border-radius: 10px; border-left: 5px solid #FF5252; margin-bottom: 20px;">
        ...
        <p style="color: #E0E0E0; font-size: 14px; line-height: 1.6;">
            日経先物が日経終値を大きく下回っています。明日は安く寄り付くリスクが高く、ポジション縮小を検討すべき地合いです。地政学リスクとインフレ懸念が市場を二分する中、米国株の堅調と一部セクターの個別材料は買いを誘ったが、米国株全体としては持続的な強さには欠け、明日はギャップダウンへの警戒が必要である。
        </p>
    </div>
    """, unsafe_allow_html=True)
    
    
    st.title("📋 米国株セクター別 騰落ダッシュボード")
    st.info(f"**ステータス:** {status_msg}")
    
    st.markdown("米国を代表する主要約500銘柄（TOPIX 500等）のデータを集計し、11業種別の動向を可視化しています。")
    ...
    with col1:
        st.header("🗺️ 米国株 全体ヒートマップ (主要約500銘柄)")
    with col2:
    ...
    fig = px.treemap(
        df_heatmap,
        path=[px.Constant("米国市場 (主要銘柄)"), "Sector", "Name"],
        values="Size",  # 売買代金ベース
        color="Change", # 騰落率ベース
    ...
    fig.update_traces(
        ...
        hovertemplate="<b>%{label}</b><br>価格: $%{customdata[1]:.2f}<br>騰落率: %{customdata[0]}<extra></extra>"
    )
    ...
    # 
    # if st.button("⏪ データ取得＆リプレイ再生", key="us_replay_btn"):
    #     with st.spinner("1日分の詳細データを取得・構築しています... (約10〜30秒)"):
    #         from us_sector_data_v2 import fetch_us_intraday_replay_data
    #         frames, times = fetch_us_intraday_replay_data()
    #         
    #     if not frames:
    ...
    selected_sector = st.selectbox("セクターを選択", sector_list, label_visibility="collapsed", key="us_sector_select")
    ...
    if selected_sector:
        ...
        event = st.dataframe(
            ...
            key=f"us_table_selection_{current_sector}"
        )
